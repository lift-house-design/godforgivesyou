...
class Authentication extends App_Controller
{
	...
	public function login_json()
	{
		...
		if($this->user->logged_in)
			$this->_json(['user' => $this->user->data['id']]);

		$email = $this->input->post('email');
		$pass = $this->input->post('pass');

		if($this->valid->email($email))
			$this->_json(['error' => 'Invalid Email Address']);
		if(empty($pass))
			$this->_json(['error' => 'Password is Empty']);

		$res = $this->db->where('email',$email)->select('id,password')->get('user')->row_array();
		if(empty($res))
		{
			...
			$this->db->insert('user', [
				'email' => $email, 
				'password' => sha1($pass), 
				'confirm_code' => $verification
			]);
			
			$user = $this->db->insert_id();
			if(empty($user))
				$this->_json(['error' => 'Internal Error']);
		...
		elseif(sha1($pass) !== $res['password'])
		{
			$this->_json(['error' => 'Incorrect Password']);
		}
		else
		{
			$user = $res['id'];
		...
		$user_data = $this->db->where('id', $user)->get('user')->row_array();
